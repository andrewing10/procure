 ...
 *
 * 目标：把用户口语搜词压成可搜的产品/品类名词，避免整句进 Serper。
 *
 * 例：
 *   "新加坡有没有电视机"     → "电视机"
...
/** 搜索国前缀（仅当后面紧跟口语意图时剥离，避免误伤「日本大米」） */
const GEO_NAME =
  '新加坡|日本|韩国|中国|美国|英国|德国|法国|澳洲|澳大利亚|马来西亚|泰国|越南|印尼|印度尼西亚|菲律宾|印度|台湾|香港|澳门|' +
  'Singapore|Japan|Korea|China|USA|United\\s+States|UK|United\\s+Kingdom|Germany|France|Australia|Malaysia|Thailand|Vietnam|Indonesia|Philippines|India|Taiwan|Hong\\s+Kong';
...
function sanitizeDiscoveryCategory(category) {
  ...
  // 1) 「新加坡有没有X」/「SG 有没有 X」：国家+意图一起剥（只剥搜索国，不剥品类里的国名）
  const geoIntent = new RegExp(
    `^(?:${GEO_NAME}|${GEO_ISO})\\s*(?:${COLLOQUIAL})\\s*`,
    'i',
  );
  ...
  // 3) 残留句首 ISO 国家码（无意图时少见）：「SG potatoes」→ 保留 potatoes 需更谨慎，仅当后面是中文产品时
  cleaned = cleaned.replace(new RegExp(`^(?:${GEO_ISO})\\s+(?=[\\u4e00-\\u9fff])`, 'i'), '').trim();
